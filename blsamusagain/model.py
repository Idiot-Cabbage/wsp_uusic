...
class Model:
    def __init__(self):
        ...
        self.config = get_config(self.args)
          ...
          # 初始化 SAMUS 模型
        self.model = ModelClass(self.config, prompt=self.args.prompt)
        # 加载预训练权重
        if os.path.exists('exp_out/trial_1/best_model.pth'):
            self.model.load_from_self('exp_out/trial_1/best_model.pth')

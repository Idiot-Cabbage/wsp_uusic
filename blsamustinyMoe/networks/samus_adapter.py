...
class SAMUSAdapter(nn.Module):
    ...
    def __init__(self, config, prompt=False):
        ...
        self.samus_args = self.create_tiny_samus_args()
        
        # 初始化 SAMUS 分割模型
        self.samus_model = moesamus_model_registry['vit_t'](
            args=self.samus_args, 
            checkpoint=self.samus_args.sam_ckpt,
            vit_checkpint =self.samus_args.tiny_vit_ckpt
        )
        ############################################################################################################
        img_size=224
        patch_size=16
    ...
    def create_tiny_samus_args(self):
        """创建 SAMUS 所需的参数"""
        class SAMUSArgs:
            def __init__(self):
                self.modelname = 'SAMUS'
                ...
                self.vit_name = 'vit_t'
                self.sam_ckpt = '/root/autodl-tmp/wsp_uusic/KTD/SAMUS-main/checkpoints/sam_vit_b_01ec64.pth'
                self.tiny_vit_ckpt = '/root/autodl-tmp/wsp_uusic/fjh_blsamus_tiny/pretrained_ckpt/tiny_vit_21m_22k_distill.pth'
                self.batch_size = 1
                self.device = 'cuda'

...
class Samus(nn.Module):
    ...
    def __init__(
        ...
        for i, blk in enumerate(self.image_encoder.layers[1].blocks):
            for p in blk.mlp.parameters():
                p.requires_grad = True
        for i, blk in enumerate(self.image_encoder.layers[2].blocks):
            for p in blk.mlp.parameters():
                p.requires_grad = True
        for i, blk in enumerate(self.image_encoder.layers[3].blocks):
            for p in blk.mlp.parameters():
                p.requires_grad = True
    ...
    def forward(
        ...
        imge= self.image_encoder(imgs)#(4,256,32,32)

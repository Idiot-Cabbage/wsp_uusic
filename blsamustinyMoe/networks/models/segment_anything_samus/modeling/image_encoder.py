...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 处理单通道输入
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)  # b c h w 4 3 224 224
        
        # 生成 cnnx
        cnnx = self.cnn_embed(x)  # b h w c 4 14 14 768
        x = self.patch_embed(x)  # b h w c  4 32 32 768
        x = self.input_Adapter(x)# 4 32 32 768

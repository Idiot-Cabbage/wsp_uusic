...
class Samus(nn.Module):
    ...
    def __init__(
        ...
        for name, param in self.mask_decoder.named_parameters():
            if "output_hypernetworks_mlps" in name or "iou_prediction_head" in name:
                param.requires_grad = True  # 解冻MoE层
            else:
                param.requires_grad = False  # 保持其他层冻结
    ...
    def forward(
        ...
        low_res_masks, _ ,router_logits= self.mask_decoder( # low_res_mask b 1 128 128
        ...
        outputs = {"low_res_logits": low_res_masks, "masks": masks,"router_logits":router_logits}

...
from utils import omni_seg_test,switch_load_balancing_loss
        ...
        for i_batch, sampled_batch in tqdm(enumerate(trainloader_seg)):
            ...
            if args.prompt:
                ...
                nature_prompt = torch.tensor(np.array(sampled_batch['nature_prompt'])).permute([
                    1, 0]).float().to(device=device)
                (x_seg, _, _,router_logits) = model((image_batch, position_prompt, task_prompt, type_prompt, nature_prompt))
            else:
                # print('进入')
                (x_seg, _, _,router_logits) = model(image_batch)
            ...
            logging.info('global iteration %d and seg iteration %d : loss : %f loss_moe : %f' %
                         (global_iter_num, seg_iter_num, loss.item(), moe_loss.item()))
